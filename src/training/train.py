...
def train_one_epoch_None_neg(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    """
        neg image, neg text 둘 다 사용하는 경우
    """
...
def train_one_epoch_neg_text_image(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    """
        neg image, neg text 둘 다 사용하는 경우
    """
    device = torch.device(args.device)
...
def train_one_epoch_neg_text(model, data, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    """
        neg image 없고 only neg text만 사용하는 경우 
    """
    device = torch.device(args.device)
    autocast = torch.cuda.amp.autocast if args.precision == 'amp' else suppress
    model.train()
...
def evaluate_None_neg(model, data, epoch, args, tb_writer=None):
    metrics = {}
...
def evaluate_neg_text_image(model, data, epoch, args, tb_writer=None):
    metrics = {}
    if not is_master(args):
        return metrics
    device = torch.device(args.device)
...
def evaluate_neg_text(model, data, epoch, args, tb_writer=None):
    metrics = {}
    if not is_master(args):
        return metrics
    device = torch.device(args.device)
